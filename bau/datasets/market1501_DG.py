...
class Market1501_DG(BaseImageDataset):
    """
    Market1501
    Reference:
    ...
    def __init__(self, root, verbose=True, **kwargs):
        super(Market1501_DG, self).__init__()
    ...
    def _process_dir_DG(self, dir_paths, relabel=False):
        ...
        for name, dir_path in dir_paths.items():
            ...
            for img_path in img_paths:
                pid, _ = map(int, pattern.search(img_path).groups())
                if pid == -1 or pid == 0: continue # junk and background images are just ignored for training
                pid = name + str(pid)
                pid_container.add(pid)
        pid2label = {pid: label for label, pid in enumerate(pid_container)}
