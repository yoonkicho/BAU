...
class MobileNetV2(nn.Module):
    ...
    def __init__(
        ...
        self.fc = self._construct_fc_layer(
            num_features, self.feature_dim, dropout
        )
    ...
    def _construct_fc_layer(self, num_features, input_dim, dropout=None):
        """Constructs fully connected layer.
        Args:
            num_features (list or tuple): dimensions of fc layers, if None, no fc layers are constructed
            input_dim (int): input dimension
            dropout (float): dropout probability, if None, dropout is unused
        """
        ...
        assert isinstance(
            num_features, (list, tuple)
        ), 'num_features must be either list or tuple, but got {}'.format(
            type(num_features)
        )

        layers = []
        for dim in num_features:
...
def mobilenetv2_x1_0(num_classes, pretrained=True, **kwargs):
    model = MobileNetV2(
        num_classes,
        width_mult=1,
        num_features=None,
...
def mobilenetv2_x1_4(num_classes, pretrained=True, **kwargs):
    model = MobileNetV2(
        num_classes,
        width_mult=1.4,
        num_features=None,
